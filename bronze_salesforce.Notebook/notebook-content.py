...
# Registry-driven ingestion from Salesforce using the REST API (SOQL queries)
# with OAuth2 Username-Password flow. Supports incremental loads via
# `SystemModstamp` watermark and full loads. Handles pagination via `nextRecordsUrl`.
# 
# Registry entry example:
# ```python
# 'sf_opportunities': {
#     'connector_type':   'salesforce',
#     'kv_username':      'sf-api-user',
#     'kv_password':      'sf-api-pass',
#     'kv_client_id':     'sf-connected-app-id',
#     'kv_client_secret': 'sf-connected-app-secret',
#     'kv_security_token':'sf-api-security-token',
#     'sf_instance_url':  'https://yourorg.my.salesforce.com',
#     'sf_object':        'Opportunity',
#     'soql_fields':      ['Id','Name','Amount','StageName','CloseDate','SystemModstamp'],
#     'soql_where':       '',                  # additional WHERE clause (optional)
#     'api_version':      'v59.0',
#     'bulk_api':         False,               # True for >100k rows
#     'bronze_table':     'salesforce/opportunities',
...
log = logging.getLogger('bronze_salesforce')
...
def sf_login(cfg):
    ...
    return token_data['access_token'], token_data['instance_url'], api_version

def build_soql(cfg, watermark_val):
    fields    = ', '.join(cfg['soql_fields'])
    sf_object = cfg['sf_object']
    wm_col    = cfg.get('watermark_col', 'SystemModstamp')
    extra_where = cfg.get('soql_where', '')

    conditions = []
    if cfg['load_type'] == 'incremental' and watermark_val != '1900-01-01T00:00:00Z':
        conditions.append(f"{wm_col} > {watermark_val}")
    if extra_where:
        conditions.append(f'({extra_where})')

    where = f" WHERE {' AND '.join(conditions)}" if conditions else ''
    return f'SELECT {fields} FROM {sf_object}{where} ORDER BY {wm_col} ASC'

def fetch_soql_records(cfg, watermark_val):
    access_token, instance_url, api_version = sf_login(cfg)
    headers = {'Authorization': f'Bearer {access_token}', 'Accept': 'application/json'}
    soql    = build_soql(cfg, watermark_val)
    url     = f'{instance_url}/services/data/{api_version}/query?q={requests.utils.quote(soql)}'
    all_records = []
    ...
    while url:
        ...
        log.info(f'[SF] fetched {len(all_records)} records so far...')
...
def add_audit_cols(df, cfg, watermark_val):
    ...
    org = cfg['sf_instance_url'].replace('https://', '').split('.')[0]
    return df \
        .withColumn('_ingested_at',       current_timestamp()) \
        .withColumn('_source_system',     lit(f'Salesforce_{org}')) \
        .withColumn('_source_table',      lit(cfg['sf_object'])) \
...
sf_sources = get_sources_by_type('salesforce')
summary = []
...
for table_name, cfg in sf_sources.items():
    ...
    try:
        ...
        records = fetch_soql_records(cfg, wm)
        if not records:
        ...
        records_clean = [{k: (str(v) if not isinstance(v, (str, int, float, bool, type(None))) else v)
                          for k, v in r.items()} for r in records]
        df_raw = spark.createDataFrame(records_clean)
