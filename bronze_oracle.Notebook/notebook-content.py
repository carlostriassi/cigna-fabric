...
# # Bronze: Oracle Database ingestion
# Registry-driven JDBC ingestion from Oracle DB with watermark, schema drift
# detection, parallel partitioning, quarantine, and Delta append.
# 
# Driver JAR required: `com.oracle.database.jdbc:ojdbc8:21.x.x` (add to cluster libraries).
# 
# Registry entry example:
# ```python
# 'oracle_orders': {
#     'connector_type': 'oracle',
#     'kv_user_key':    'ora-erp-user',
#     'kv_pass_key':    'ora-erp-pass',
#     'jdbc_host':      'prod-oracle.company.com',
#     'jdbc_port':      1521,
#     'jdbc_service':   'ERPDB',          # service name (preferred over SID)
#     'source_schema':  'APP_SCHEMA',
#     'source_table':   'ORDERS',
#     'bronze_table':   'oracle/orders',
#     'load_type':      'incremental',
#     'watermark_col':  'LAST_UPDATE_DATE',
...
log = logging.getLogger('bronze_oracle')
...
# CELL 3 — Oracle JDBC connection builder
def build_jdbc_url(cfg):
    host    = cfg['jdbc_host']
    port    = cfg.get('jdbc_port', 1521)
    service = cfg.get('jdbc_service')
    sid     = cfg.get('jdbc_sid')
    if service:
...
def build_jdbc_options(cfg, watermark_val):
    ...
    if is_incr and wm_col and watermark_val != '1900-01-01 00:00:00':
        # Oracle uses TO_DATE for timestamp comparisons
        pushdown = (f"(SELECT * FROM {schema}.{table} "
                    f"WHERE {wm_col} > TO_TIMESTAMP('{watermark_val}', "
                    f"'YYYY-MM-DD HH24:MI:SS')) src")
    else:
...
def add_audit_cols(df, cfg, watermark_val):
    ...
    return df \
        ...
        .withColumn('_source_system',     lit(f"ORA_{cfg.get('jdbc_service', cfg.get('jdbc_sid', 'DB'))}")) \
...
oracle_sources = get_sources_by_type('oracle')
summary = []

for table_name, cfg in oracle_sources.items():
    log.info(f'\n=== {table_name} ===')
    wm = None
    try:
