...
# # Bronze: SharePoint ingestion
# Registry-driven Microsoft Graph API ingestion for SharePoint Online lists.
# Uses OAuth2 client-credentials flow (Azure AD app registration), handles
# `@odata.nextLink` pagination, and writes all pages to Bronze as Delta append.
# 
# **No code changes needed to add a new list** — add an entry to
# `config/source_registry.py` with `connector_type: sharepoint`.
# 
# Registry entry example:
# ```python
# 'sp_project_tracker': {
#     'connector_type': 'sharepoint',
#     'kv_tenant_id':   'sp-tenant-id',
#     'kv_client_id':   'sp-app-client-id',
#     'kv_client_secret': 'sp-app-client-secret',
#     'site_id':        'yourorg.sharepoint.com,<site-guid>,<web-guid>',
#     'list_id':        '<list-guid>',
#     'select_fields':  ['id','Title','Status','DueDate','Modified'],
#     'expand_fields':  ['fields'],
#     'bronze_table':   'sharepoint/project_tracker',
#     'load_type':      'incremental',
#     'watermark_col':  'Modified',
#     'primary_keys':   ['id'],
# }
# ```
...
log = logging.getLogger('bronze_sharepoint')
...
# CELL 3 — Graph API OAuth2 + paginated list reader
...
def fetch_list_items(cfg, watermark_val):
    token   = get_graph_token(cfg)
    headers = {'Authorization': f'Bearer {token}', 'Accept': 'application/json'}
    site_id = cfg['site_id']
    list_id = cfg['list_id']

    select  = ','.join(cfg.get('select_fields', []))
    expand  = ','.join(cfg.get('expand_fields', ['fields']))
    filter_clause = ''
    if cfg['load_type'] == 'incremental' and watermark_val != '1900-01-01T00:00:00Z':
        wm_col = cfg.get('watermark_col', 'Modified')
        filter_clause = f"&$filter=fields/{wm_col} gt '{watermark_val}'"

    url = (f"{GRAPH_BASE}/sites/{site_id}/lists/{list_id}/items"
           f"?$expand={expand}&$select={select}{filter_clause}&$top=5000")
    ...
    while url:
        ...
        log.info(f'[SP] fetched {len(all_items)} items so far...')

    return all_items

def flatten_sp_item(item):
    flat = {'id': item.get('id'), 'created_dt': item.get('createdDateTime'),
...
def add_audit_cols(df, cfg, watermark_val):
    ...
    site_id = cfg['site_id'].split(',')[0] if ',' in cfg['site_id'] else cfg['site_id']
    return df \
        .withColumn('_ingested_at',       current_timestamp()) \
        .withColumn('_source_system',     lit(f'SharePoint_{site_id}')) \
        .withColumn('_source_table',      lit(cfg['list_id'])) \
...
sp_sources = get_sources_by_type('sharepoint')
summary = []

for table_name, cfg in sp_sources.items():
    log.info(f'\n=== {table_name} ===')
    wm = None
    try:
        wm = read_watermark(table_name) if cfg['load_type'] == 'incremental' else '1900-01-01T00:00:00Z'
        items = fetch_list_items(cfg, wm)
        if not items:
            log.info(f'[BRONZE] {table_name}: 0 new items')
            write_watermark(table_name, str(datetime.utcnow()), 0, 'SUCCESS')
            summary.append({'table': table_name, 'rows': 0, 'status': 'SUCCESS'})
            continue

        flat_items = [flatten_sp_item(i) for i in items]
        df_raw = spark.createDataFrame(flat_items)
        ...
        wm_col = cfg.get('watermark_col', 'modified_dt')
