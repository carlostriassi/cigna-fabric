...
# `config/source_registry.py` with `connector_type: postgresql`.
# 
# Driver JAR required: `org.postgresql:postgresql:42.x.x` (add to cluster libraries).
...
log = logging.getLogger('bronze_postgresql')
...
# CELL 3 — PostgreSQL JDBC connection builder
def build_jdbc_url(cfg):
    host = cfg['jdbc_host']
    port = cfg.get('jdbc_port', 5432)
    db   = cfg['jdbc_db']
    ssl  = cfg.get('ssl', True)
    return f'jdbc:postgresql://{host}:{port}/{db}?ssl={str(ssl).lower()}'
...
def add_audit_cols(df, cfg, watermark_val):
    ...
    return df \
        ...
        .withColumn('_source_system',     lit(f"PG_{cfg['jdbc_db']}")) \
...
pg_sources = get_sources_by_type('postgresql')
summary = []

for table_name, cfg in pg_sources.items():
    log.info(f'\n=== {table_name} ===')
    wm = None
    try:
